...
# 字符串：char varchar  区别：char 定长 char(10)
...
#     }
# ]时间限制：毫秒    空间限制：字节
delete from tb_question  where question_id = 1;
...
# 、撤销发布
#
# C端：列表（未开始、历史） 、报名参赛、我的比赛 、参加竞赛
# （竞赛倒计时、竞赛内题目切换 、完成竞赛） 、竞赛练习、查看排名
# 、我的消息 是否开赛 不需要加在数据库中，我们实时计算就可以 报名参赛：未开始、未报名
